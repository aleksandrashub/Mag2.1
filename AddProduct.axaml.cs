...
public partial class AddProduct : Window
{
    ...
    private bool checkValues(double pr, int quan)
    {
        ...
        else
        {
            check = false;
            pricePr.Text = "";
            MessageBoxManager.GetMessageBoxStandard(text: "Цена товара не может быть отрицательной. Введите заново.", title: "Ошибка").ShowAsync();
        }
        ...
        if (quan < 0)
        {
            check = false;
            kolvoPr.Text = "";
            MessageBoxManager.GetMessageBoxStandard(text: "Количество товара не может быть отрицательным. Введите заново.", title: "Ошибка").ShowAsync();
        }
        return check;
    }
